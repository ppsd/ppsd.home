...
ensureColumn('purchase_requests', 'source_pref', 'TEXT')  // ช่องทางจัดซื้อที่ผู้ขอเลือก: shop | online | both
ensureColumn('purchase_requests', 'online_options', 'TEXT') // ผล AI หาของออนไลน์ (JSON)
...
db.exec(`CREATE TABLE IF NOT EXISTS doc_images (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  kind TEXT, doc_id INTEGER, key TEXT, png BLOB, created TEXT
)`) // รูปเอกสารที่ระบบสร้าง (PNG) สำหรับส่งเข้า LINE / ดูย้อนหลัง
...
ensureColumn('vendors', 'kind', 'TEXT')                 // หมวด: 'ผู้ขาย' (วัสดุ/ของ) หรือ 'ผู้รับเหมา' (ค่าแรง/รับช่วง) — ว่าง = ผู้ขาย
ensureColumn('vendors', 'address', 'TEXT')              // ที่อยู่ร้าน/ช่าง (ใช้ในเอกสาร + ติดต่อ)
ensureColumn('vendors', 'category', 'TEXT')             // หมวดสินค้า/งาน เช่น วัสดุก่อสร้าง ไฟฟ้า ประปา งานปูน
...
// ===== ราคากลางค่าแรงช่าง (Standard Labor Cost) — ใบเทียบราคากลางของฝ่ายประเมินราคาและจัดซื้อ =====
// ใช้เป็น "ราคาแนะนำ" ตอนจ้างช่าง: เลือกหมวดงาน → ระบบโชว์ราคากลาง → ราคาตกลงต้องไม่เกิน (เตือนถ้าเกิน)
db.exec(`CREATE TABLE IF NOT EXISTS labor_rates (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  grp TEXT,            -- 'เหมายกหลัง' | 'แยกงาน'
  seq INTEGER,         -- ลำดับในใบราคากลาง
  name TEXT,           -- หมวดหมู่ / รายการ เช่น งานก่อผนังอิฐแดง
  variant TEXT,        -- แบบย่อย เช่น รวมไม้แบบ / ไม่รวมไม้แบบ ('' = ไม่มี)
  price_min REAL,      -- ราคากลางต่ำสุด (บาท/หน่วย) — 0 = เสนอราคา
  price_max REAL,      -- ราคากลางสูงสุด (บาท/หน่วย)
  unit TEXT,           -- ตร.ม. / เมตร / ชุด / บาน / ห้อง / เหมา / หลัง
  note TEXT, active INTEGER DEFAULT 1, updated TEXT
)`)
ensureColumn('contractors', 'labor_rate_id', 'INTEGER') // หมวดงานที่จ้าง (อ้าง labor_rates.id)
ensureColumn('contractors', 'qty', 'REAL')              // ปริมาณงานที่ตกลง (ตาม unit ของหมวด)
...
ensureColumn('contractors', 'vendor_id', 'INTEGER')
ensureColumn('users', 'line_uid', 'TEXT') // LINE userId ที่ผูกกับบัญชี (สั่งงาน/รับงานผ่านบอท LINE)     // อ้างทะเบียนผู้รับเหมา (vendors.kind='ผู้รับเหมา') ถ้าเลือกจากทะเบียน
if (db.prepare('SELECT COUNT(*) c FROM labor_rates').get().c === 0) {
  ...
  const rows = [
    // ราคาค่าแรงแบบเหมายกหลัง
    ['เหมายกหลัง', 1, 'เหมายกหลัง ค่าของ+ค่าแรง', '', 13500, 13500, 'ตร.ม.', 'ต้องเป็นบริษัท มี VAT'],
    ['เหมายกหลัง', 2, 'เหมายกหลัง ค่าแรงอย่างเดียว', '', 3500, 4000, 'ตร.ม.', ''],
    // ราคาค่าแรงแยกแต่ละงาน
    ['แยกงาน', 1, 'งานโครงสร้าง', 'ไม่รวมไม้แบบ', 1000, 1000, 'ตร.ม.', ''],
    ['แยกงาน', 1, 'งานโครงสร้าง', 'รวมไม้แบบ', 1200, 1200, 'ตร.ม.', ''],
    ['แยกงาน', 2, 'งานโครงหลังคา', '', 160, 160, 'ตร.ม.', ''],
    ['แยกงาน', 3, 'งานมุงหลังคา', '', 160, 160, 'ตร.ม.', ''],
    ['แยกงาน', 4, 'งานเชิงชาย', '', 100, 100, 'เมตร', ''],
    ['แยกงาน', 5, 'งานก่อผนังอิฐแดง', '', 120, 150, 'ตร.ม.', ''],
    ['แยกงาน', 6, 'งานฉาบผนัง', '', 100, 120, 'ตร.ม.', ''],
    ['แยกงาน', 7, 'งานเททับหลัง', '', 50, 50, 'เมตร', ''],
    ['แยกงาน', 8, 'งานจับเซี้ยม', 'สำเร็จ', 25, 25, 'เมตร', ''],
    ['แยกงาน', 8, 'งานจับเซี้ยม', 'สด', 35, 35, 'เมตร', ''],
    ['แยกงาน', 9, 'งานตั้งวงกบ', '', 200, 200, 'ชุด', ''],
    ['แยกงาน', 10, 'งานฝ้าฉาบเรียบ', 'ภายใน', 220, 250, 'ตร.ม.', ''],
    ['แยกงาน', 10, 'งานฝ้าฉาบเรียบ', 'ภายนอก (กันชื้น)', 260, 280, 'ตร.ม.', ''],
    ['แยกงาน', 11, 'รางซ่อนม่าน', '', 200, 200, 'เมตร', ''],
    ['แยกงาน', 12, 'งานระบบไฟฟ้า', '', 0, 0, 'เหมา', 'เสนอราคา'],
    ['แยกงาน', 13, 'งานปูกระเบื้องพื้น', '', 180, 200, 'ตร.ม.', ''],
    ['แยกงาน', 14, 'งานปูกระเบื้องผนัง (ฉาบเอง)', '', 250, 280, 'ตร.ม.', ''],
    ['แยกงาน', 15, 'งานก่อเคาน์เตอร์ครัว (ก่อ+ปูกระเบื้อง+หน้าบาน+ซิงค์)', '', 2500, 2500, 'เมตร', ''],
    ['แยกงาน', 16, 'งานปูลูกบันได', '', 250, 250, 'เมตร/ขั้น', ''],
    ['แยกงาน', 17, 'งานราวระเบียงกันตก / ราวบันได', '', 800, 800, 'เมตร', ''],
    ['แยกงาน', 18, 'งานติดตั้งประตู', 'ลูกบิดธรรมดา', 500, 500, 'บาน', ''],
    ['แยกงาน', 18, 'งานติดตั้งประตู', 'ตลับ', 1000, 1000, 'บาน', ''],
    ['แยกงาน', 19, 'งานสุขาภิบาล / ประปา', '', 0, 0, 'เหมา', 'เสนอราคา'],
    ['แยกงาน', 20, 'งานติดตั้งสุขภัณฑ์ (เหมาย่อห้อง)', '', 2500, 2500, 'ห้อง', ''],
    ['แยกงาน', 21, 'งานอลูมิเนียม (เหมายกหลัง)', '', 0, 0, 'หลัง', 'เสนอราคา'],
    ['แยกงาน', 22, 'งานหล่อเคาน์เตอร์ห้องน้ำ-ครัว (หล่ออย่างเดียว)', '', 1000, 1000, 'เมตร', ''],
    ['แยกงาน', 23, 'งานทาสีบ้าน (สีสกิม)', '', 0, 0, 'เหมา', 'เสนอราคา'],
    ['แยกงาน', 24, 'งานพื้น SPC', '', 0, 0, 'เหมา', 'เสนอราคา'],
  ]
  db.transaction(() => rows.forEach((r) => ins.run(...r, today)))()
}
...
// ผังบัญชีมาตรฐาน (ธุรกิจรับเหมาก่อสร้าง SME ไทย) — seed แบบ idempotent ตามรหัส
{
  const CHART = [
    // สินทรัพย์
    ['1010', 'เงินสด', 'asset'], ['1020', 'เงินฝากธนาคาร', 'asset'], ['1030', 'เงินสดย่อย', 'asset'], ['1031', 'เงินสดย่อย-ค่าน้ำมันรถ', 'asset'],
