...
test('เงินสดย่อย: ผูกบ้านได้ — รายการ/ใบสรุปแสดงบ้าน · ต้นทุนไปรวมที่บ้านในกำไรรายโครงการ · รหัสบ้านผิดถูกกัน', async () => {
  ...
  assert.equal((await POST('/petty-cash/expense', { cat: 'วัสดุ/ของใช้หน้างาน', item: 'ตะปู+ลวดผูกเหล็ก', amount: 351, house_code: 'XX-NOPE' })).status, 400, 'บ้านไม่มีต้องกัน')
  ...
  const ex = await POST('/petty-cash/expense', { cat: 'วัสดุ/ของใช้หน้างาน', item: 'ตะปู+ลวดผูกเหล็ก', amount: 351, house_code: 'TS-01' })
  assert.equal(ex.status, 200, JSON.stringify(ex.data))
  const row = ex.data.rows.find((r) => r.memo === 'ตะปู+ลวดผูกเหล็ก')
  assert.ok(row, 'ต้องเห็นรายการในความเคลื่อนไหว'); assert.equal(row.house_code, 'TS-01'); assert.equal(row.house_name, 'บ้านเทสต์'); assert.equal(row.credit, 351)
  ...
  assert.equal(Math.round((pnl.cost + pnl.expense - costBefore) * 100) / 100, 351, 'ต้นทุนบ้านต้องเพิ่มเท่ายอดเงินสดย่อยที่ผูกบ้าน')
  const stmt = (await GET('/petty-cash/statement')).data
  const sr = stmt.rows.find((r) => r.memo === 'ตะปู+ลวดผูกเหล็ก')
  assert.equal(sr.house_code, 'TS-01'); assert.equal(sr.house_name, 'บ้านเทสต์'); assert.equal(sr.out, 351)
})
...
test('LINE: ส่งต่อลิงก์จากผล AI หาร้าน + "เอาร้านนี้" → ออก PO จากใบที่ถูกต้อง แม้บริบทค้างอยู่ใบอื่น · พิมพ์เลข PR สลับใบ · "เอาร้านที่ 2"', async () => {
  const hook = (uid, ev) => api('POST', '/line/webhook', { events: [{ replyToken: 'r1', source: { type: 'user', userId: uid }, ...ev }] })
  const msg = (uid, text) => hook(uid, { type: 'message', message: { type: 'text', text } })
  const wait = (ms = 600) => new Promise((r) => setTimeout(r, ms))
  const acc = (await GET('/users')).data.find((u) => u.username === 'acctest')
  const code = await POST('/line-link/code', { user_id: acc.id }); await msg('Uacc', code.data.code); await wait()
  // ใบ A: AI หาร้านออนไลน์แล้ว (ผลจำลอง 3 ร้าน) · ใบ B: ใบอื่นที่บัญชีกำลังรอรูปใบเสนอราคาอยู่ (บริบทค้างที่ใบนี้ = เคสจริงที่พลาด)
  const a = await POST('/purchase-requests', { house: 'บ้านเทสต์', items: [{ desc: 'สีทาบ้าน TOA', qty: 4, unit: 'ถัง', price: 1200 }], source_pref: 'online' })
  await POST(`/approve/pr/${a.data.id}`)
  assert.equal((await POST(`/purchase-requests/${a.data.id}/online-search`, { notify: false })).status, 200)
  const b = await POST('/purchase-requests', { house: 'บ้านเทสต์', items: [{ desc: 'ท่อ PVC 4 นิ้ว', qty: 5, unit: 'เส้น', price: 300 }] })
  await POST(`/approve/pr/${b.data.id}`)
  await msg('Uacc', `ใบเสนอราคา ${b.data.no}`); await wait()
  // ส่งต่อข้อความผลค้นหา (ชื่อสินค้า + ลิงก์ Lazada) พร้อม "เอาร้านนี้ค่ะ" → ต้องออก PO ให้ใบ A ร้าน Lazada 1190×4
  await msg('Uacc', 'สีทาบ้านเอาร้านนี้ค่ะ\nTOA ชิลด์ ขายในราคา ฿1,190 ซื้อได้ในแอป Lazada https://www.lazada.co.th/p/2'); await wait(1200)
  let ra = (await GET('/purchase-requests')).data.find((r) => r.id === a.data.id)
  assert.equal(ra.po_vendor, 'Lazada', 'ต้องออก PO ให้ใบที่ AI หาร้าน ไม่ใช่ใบที่ค้างในบริบท'); assert.ok(ra.po_no)
  const poA = (await GET('/purchase-orders')).data.find((p) => p.no === ra.po_no); assert.equal(poA.amount, 4760)
  const rb = (await GET('/purchase-requests')).data.find((r) => r.id === b.data.id); assert.ok(!rb.po_no, 'ใบ B ต้องไม่ถูกออก PO')
  // พิมพ์เลข PR เฉยๆ = สลับบริบทมาใบนั้น แล้ว "เอาร้านที่ 1" → เปลี่ยนเป็น HomePro (PO เดิมยังไม่อนุมัติ ถูกยกเลิกแทน)
  await msg('Uacc', a.data.no); await wait()
  await msg('Uacc', 'เอาร้านที่ 1'); await wait(1200)
  ra = (await GET('/purchase-requests')).data.find((r) => r.id === a.data.id)
  assert.equal(ra.po_vendor, 'HomePro'); assert.notEqual(ra.po_no, poA.no)
  assert.equal((await GET('/purchase-orders')).data.find((p) => p.no === poA.no).status, 'ยกเลิก')
  // ลิงก์ที่ไม่ตรงกับตัวเลือกใดๆ → ไม่ออก PO เพิ่ม
  const nPo = (await GET('/purchase-orders')).data.length
  await msg('Uacc', 'เอาร้านนี้ https://www.example.com/x'); await wait()
...
test('เงินสดย่อย/น้ำมัน: ยอดแต่ละบรรทัดปัดเศษสตางค์เป็นบาทเต็ม (≥50 สต. ปัดขึ้น, <50 ปัดลง) แล้วค่อยคิด VAT', async () => {
